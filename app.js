class JSONFormatter {
  ...
  // In your JSONFormatter class:
  formatJSON(input) {
    try {
      const parsed = JSON.parse(input);
      this.formattedJSON = JSON.stringify(parsed, null, 2);
      // --- New tree-rendering logic for collapsible in output ---
      this.jsonOutput.innerHTML = "";
      const tree = this.buildCollapsible(parsed);
      this.jsonOutput.appendChild(tree);

      this.updateStatus("input", "Valid JSON", "success");
      this.updateStatus(
        "output",
        `Formatted successfully (${this.getJSONStats(parsed)})`,
        "success"
      );
    } catch (error) {
...
buildCollapsible(obj) {
    if (obj !== null && typeof obj === "object") {
        // Handle both arrays and objects here—no separate array recursion
        const isArray = Array.isArray(obj);

        const wrapper = document.createElement("div");
        wrapper.className = "json-collapsible-group";

        const arrow = document.createElement("span");
        arrow.className = "json-collapsible-arrow open";
        arrow.textContent = "▼";
        arrow.tabIndex = 0;

        const opener = document.createElement("span");
        opener.className = "json-collapsible-opener";
        opener.textContent = isArray ? "[" : "{";

        const closer = document.createElement("span");
        closer.className = "json-collapsible-closer";
        closer.textContent = isArray ? "]" : "}";

        const content = document.createElement("div");
        content.className = "json-collapsible-content";

        const keys = isArray ? obj.map((_, idx) => idx) : Object.keys(obj);
        keys.forEach((k, i) => {
            const item = document.createElement("div");
            item.className = "json-collapsible-item";
            if (!isArray) {
            ...
            item.appendChild(this.buildCollapsible(obj[k]));
            if (i < keys.length - 1) item.appendChild(document.createTextNode(","));
        ...
        valSpan.className = `json-value json-value-${typeof obj}`;
        valSpan.textContent = typeof obj === "string" ? `"${obj}"` : String(obj);
        return valSpan;
    }
}
  ...
  clearOutput() {
    this.jsonOutput.innerHTML =
      '<code class="json-code">Enter JSON in the left pane to see formatted output here</code>';
    this.formattedJSON = "";
  }

  updateStatus(type, message, status = "") {
    const statusElement =
      type === "input" ? this.inputStatus : this.outputStatus;
  ...
  async copyToClipboard(type) {
    try {
      ...
      this.showMessage(
        `${type === "input" ? "Raw" : "Formatted"} JSON copied to clipboard`,
        "success"
      );
    } catch (error) {
  ...
  minifyJSON() {
    ...
    try {
      ...
      this.jsonOutput.innerHTML = `<code class="json-code">${this.highlightJSON(
        minified
      )}</code>`;
